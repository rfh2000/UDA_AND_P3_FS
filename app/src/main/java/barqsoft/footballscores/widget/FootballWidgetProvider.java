...
public class FootballWidgetProvider extends AppWidgetProvider {
    ...
    // These constants apply to the stack widget
...
//        }
        for (int i = 0; i < appWidgetIds.length; ++i) {
            ...
            Intent intent = new Intent(context, FootballRemoteViews.class);
            intent.putExtra(AppWidgetManager.EXTRA_APPWIDGET_ID, appWidgetIds[i]);
            // When intents are compared, the extras are ignored, so we need to embed the extras
            // into the data so that the extras will not be ignored.
            intent.setData(Uri.parse(intent.toUri(Intent.URI_INTENT_SCHEME)));

            // Note these lines to refer to the StackWidget specific layouts
            RemoteViews rv = new RemoteViews(context.getPackageName(), R.layout.stack_appwidget);
            ...
            Intent toastIntent = new Intent(context, FootballRemoteViews.class);
